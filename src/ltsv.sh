__enhancd::ltsv::open()
{
  command cat "${ENHANCD_ROOT}/options.ltsv" 2>/dev/null
  command cat "${ENHANCD_DIR}/options.ltsv" 2>/dev/null
  command cat "${HOME}/.config/enhancd/options.ltsv" 2>/dev/null
}
